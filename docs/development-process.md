...
セルフレビューの最低深度（T1/T2/T3）と確認項目は `scripts/review-depth.mjs` が実差分のリスク評価から機械算出する。レビューと修正の反復は「open findingが0件で収束・最大3ラウンド・2ラウンド目以降は変更hunkと影響項目に限定」のプロトコルに従う（`skills/code-review/SKILL.md`）。

Checkerは「セルフレビュー → CI → PR上のボット指摘 → 人間承認」のチェーンとし、T3相当では自己レビューだけで収束を深追いしない。失敗側の停止条件は、同一原因の失敗3回・検証手段なし・要求矛盾で停止して報告する。ユーザーの訂正・繰り返しの失敗・制御の穴は同じPRで skills/・AGENTS.md・docs/ へ書き戻す。
...
「全コマンドを毎回実行する」ことではなく、受入条件と関連する不変条件、必須確認を証明する。受入条件は要求工程でbullet化したものを全件照合し、検証した内容・未検証・残課題をPR・作業報告へ記録する。
...
ユーザー指定の完了地点に従う。PR指摘は `node scripts/collect-pr-findings.mjs` で機械収集して全件確認し、修正 or 棄却の根拠を残す。指摘対応はまとめて1 pushで行い、pushごとのCI起動を抑える。GitHubの承認・branch保護条件を満たす。
...
- **Checker chain**: 「セルフレビュー → CI → PR上のボット指摘 → 人間承認」の確認系列。自己評価は独立レビューの代替にならない。
