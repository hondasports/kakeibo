...
- レビューと修正は、open findingが0件になるか最大3ラウンドで収束させる。2ラウンド目以降の再レビュー対象は変更hunk・影響項目・前回open findingsに限定する（`skills/code-review`）。
...
- ユーザーの訂正・繰り返しの失敗・制御の穴は、その出力の修正で終わらせず、適用可能なら同じPRで `skills/`・`AGENTS.md`・`docs/` へ書き戻す。
