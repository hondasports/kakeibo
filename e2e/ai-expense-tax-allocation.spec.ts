...
test("#748 未配分の13明細を確認して税込合計・税額を保存後も維持する", async ({ page }) => {
  ...
  try {
    ...
    const saved = dialog.getByRole("region", { name: "保存内容", exact: true });
    await expect(saved).toContainText("商品の税額：未確定");
    await expect(saved).toContainText("印字額の合計：4,292円");
    await expect(saved).toContainText("商品合計：未確定");
    ...
    await expect(dialog.getByRole("region", { name: "確認すること" })).toContainText(
    ...
    for (const name of ["割引8", "割引9"]) {
      await dialog.getByRole("combobox", { name: name + "の割引対象税率", exact: true }).click();
      await page.getByRole("option", { name: "8%対象の割引", exact: true }).click();
      await expect(
        dialog.getByRole("combobox", { name: name + "の割引対象税率", exact: true }),
      ).toHaveText("8%対象の割引");
    }
    await expect(saved).toContainText("商品合計：4,662円");
    await expect(saved).toContainText("商品の税額：370円");
    await expect(saved).toContainText("差額：0円（金額一致）");
